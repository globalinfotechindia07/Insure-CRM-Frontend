...
const AddOutSourceDiagnostics = ({ getData, handleClose }) => {
  ...
  const fetchData = async () => {
    ...
    if (departmentData.data.length > 0) {
      ...
      const dept = departmentData?.data.filter(
  ...
  const handleSubmit = async e => {
    ...
    if (validateForm()) {
      try {
        const response = await post('outsourceDiagnostic-master', labData)
        if (response.lab) {
...
export default AddOutSourceDiagnostics
