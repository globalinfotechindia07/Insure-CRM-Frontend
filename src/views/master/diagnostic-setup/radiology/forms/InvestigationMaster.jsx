...
const InvestigationMaster = () => {
  ...
  const filterDataHandler = (e) => {
    ...
    const filteredData = filterData.filter((item) => {
      return (
        ...
        (item.testRange !== null && item.testRange.toLowerCase().includes(searchValue)) ||
        (item.description !== null && item.description.length > 0 && item.description.toLowerCase().includes(searchValue))
      );
    });
    let addsr = [];
  ...
  const fetchData = async () => {
    await get('department-setup').then((response) => {
      ...
      response.data.forEach((v) => {
        if (v.departmentFunction.isRadiology === true) {
          dept.push(v);
        }
      });
      setAllData(dept);
    ...
    await get('machine-radiology-master').then((response) => {
    ...
    setLoader(true);
    await get('investigation-radiology-master').then((response) => {
      let addsr = [];
      response.investigation.forEach((val, index) => {
        addsr.push({ ...val, sr: index + 1 });
      });
      setShowData(addsr);
      setFilterData(addsr);
  ...
  const deleteData = async (id) => {
    await put(`investigation-radiology-master/delete/${id}`)
      .then(() => {
        fetchData();
        // toast({
        //   title: `${data.testName} Investigation Master deleted!!`,
        //   status: "success",
        //   duration: 4000,
        //   isClosable: true,
        //   position: "bottom",
  ...
  const handleSubmit = async (e) => {
    ...
    // inputData.testType !== '' &&
    // (inputData.testType === 'None' ||
    //   (inputData.testType === 'Numeric' && inputData.testRangeStart !== '' && inputData.testRangeEnd !== '') ||
    //   (inputData.testType === 'Descriptive' && inputData.description.length > 0 && inputData.description[0].blocks.length > 0));
    ...
    if (isValid) {
      ...
      await post('investigation-radiology-master', datatoSend)
        .then(() => {
          setOpenRegistrationModal(false);
          setDescription({});
          fetchData();
          ...
          toast.success('Investigation Master Added!!');
        })
        .catch((error) => {
          // toast({
          //   title: "Something went wrong, Please try later!!",
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (isValid) {
      ...
      await put(`investigation-radiology-master/${data._id}`, datatoSend)
        .then(() => {
          setOpenEditModal(false);
          fetchData();
  ...
  const exportDataHandler = () => {
    ...
    showData.forEach((val, ind) => {
      datadd.push({
        ...
        Department: val.department ? val.department.replace(/,/g, ' ') : null,
        ...
        'Test Range': val.testType === 'Numeric' && val.testRange ? val.testRange.replace(/,/g, ' ') : null
        // Description:
        //   val.testType === 'Descriptive' && val.description?.blocks?.[0]?.data?.text
        //     ? val.description.blocks[0].data.text.replace(/,/g, ' ')
        //     : null
      });
    });
    return datadd;
  ...
  return (
    <div>
      ...
      <Modal open={openEditModal} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <div className="modal">
          <h2 className="popupHead">Update Investigation Master</h2>
