...
const EditSecondLevelNestedData = ({ fetchData, handleClose, editData, investigation }) => {
  ...
  if (isValid) {
    // Find the index of the parameter that matches the current editData.
    const updatedParameters = [...investigation.parameters]
    const editIndex = updatedParameters.findIndex(
      param => param._id === editData._id // You might need to adjust this based on a unique identifier
    )

    // Update the existing parameter if found, otherwise add it (optional based on your use case).
    if (editIndex !== -1) {
      updatedParameters[editIndex] = {
      ...
      // Optionally handle cases where the parameter was not found if necessary.
      toast.error("Data to edit not found")
      return
    }


    try {
      // Send the updated parameters array to the backend
  ...
  return (
    <div>
      <h2 className='popupHead'>Add under</h2>
...
export default EditSecondLevelNestedData
