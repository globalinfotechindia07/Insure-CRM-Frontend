...
import AddInterval from './forms/AddInterval';
import EditInterval from './forms/EditInterval';
...
const TimeInterval = () => {
  ...
  const getData = async () => {
    setLoader(true);
    await get('time-interval').then((response) => {
      let addsr = [];
      response.allInterval.forEach((val, index) => {
  ...
  const filterData = (e) => {
    ...
    const filteredData = serverData.filter((item) => {
      item.timeInterval.toLowerCase().includes(searchValue);
    });
    let addsr = [];
  ...
  const deleteData = async (id) => {
    await put(`itime-interval/delete/${id}`)
      .then(() => {
        getData();
        toast.error(`${data.timeInterval} min Deleted`);
        setOpenDeleteModal(false);
      })
      .catch((error) => {
  ...
  const columns = ['SN', 'Time Interval', 'Action'];
  const finalData =
    showData &&
    showData.map((item, ind) => {
      return {
        SN: ind + 1,
        'Time Interval': <>{item.timeInterval} min</>,
        Action: (
  ...
  return (
    <>
      <Breadcrumb title="Time Interval">
      ...
      <Card>
        <CardContent>
          ...
          <Button variant="outlined" className="global_btn" onClick={()=>navigate(-1)}>
             <MdArrowLeft/> Go back
             </Button>
          ...
          <Modal open={openRegistrationModal}>
            {type === 'add' ? (
              <AddInterval close={closeRegistration} getData={getData} />
            ) : (
              <EditInterval close={closeRegistration} editData={editData} getData={getData} />
            )}
          </Modal>
          <Modal
          ...
          >
            <div className="modal">
              <h2 className="popupHead">Delete {data.timeInterval} min delete?</h2>
