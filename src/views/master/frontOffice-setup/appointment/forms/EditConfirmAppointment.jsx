...
const EditConfirmAppointment = ({ patientDetail, close, getData }) => {
  console.log('PATIWNT DETAIL', patientDetail);
  ...
  useEffect(() => {
    ...
    const currentTime = now.toTimeString().split(' ')[0].slice(0, 5); // Format as HH:mm
  ...
  const [inputOpdData, setInputOpdData] = useState({
    packagesType: patientDetail?.packagesType || '',
    packageValidity: patientDetail?.packageValidity || '',
    marketingCommunity: patientDetail?.marketingCommunity || '',
    registrationDate: patientDetail?.registrationDate || '',
    registrationTime: patientDetail?.registrationTime || '',
    opd_regNo: patientDetail?.opd_regNo || ''
  });

  useEffect(() => {
    setInputOpdData({
      packagesType: patientDetail?.packagesType || 'd',
      packageValidity: patientDetail?.packageValidity || '',
      marketingCommunity: patientDetail?.marketingCommunity || '',
      registrationDate: patientDetail?.registrationDate || '',
      registrationTime: patientDetail?.registrationTime || '',
  ...
  const [inputData, setInputData] = useState({
    ...
    prefixId: patientDetail?.prefixId || '',
    ...
    mobile_no: patientDetail?.mobile_no || '',
    country: patientDetail?.country || '',
    state: patientDetail?.state || '',
    city: patientDetail?.city || '',
    address: patientDetail?.address || '',
    pincode: patientDetail?.pincode || '',
    birthTime: patientDetail?.birthTime || '',
    martialStatus: patientDetail?.martialStatus || '',
    aadhar_no: patientDetail?.aadhar_no || '',
    abha_no: patientDetail?.abha_no || '',
    patientPhoto: patientDetail?.patientPhoto || '',
    patientImpression: patientDetail?.patientImpression || '',
    departmentName: patientDetail?.departmentName || '',
    departmentId: patientDetail?.departmentId || '',
    consultantName: patientDetail?.consultantName || '',
    consultantId: patientDetail?.consultantId || '',
    bedAllocation: patientDetail?.bedAllocation || '',
    payeeCategory: patientDetail?.payeeCategory || '',
    payeeCategoryId: patientDetail?.payeeCategoryId || '',
    patientPayee: patientDetail?.patientPayee || '',
    patientPayeeId: patientDetail?.patientPayeeId || '',
    referBy: patientDetail?.referBy || '',
    refferName: patientDetail?.refferName || '',
    refferMobile: patientDetail?.refferMobile || '',
    relativePrifix: patientDetail?.relativePrifix || '',
    relativePrifixId: patientDetail?.relativePrifixId || '',
    relative_name: patientDetail?.relative_name || '',
    relative_mobile: patientDetail?.relative_mobile || '',
    relation: patientDetail?.relation || '',
    note: patientDetail?.note || '',
    uhidNo: patientDetail?.uhid || '',
    charityIndigent: patientDetail?.charityIndigent || '',
    charityWeaker: patientDetail?.charityWeaker || '',
    charityCamp: patientDetail?.charityCamp || '',
    tokenNo: patientDetail?.tokenNo,
    formFillingTime: patientDetail?.formFillingTime
  });

  // Update state when patientDetail changes
  ...
  const handleChange = (event, section) => {
    ...
    } else if (['aadhar_card', 'abha_card', 'cardAttachment'].includes(name)) {
  ...
  const handleRelativeRefrence = (event) => {
    ...
    if (name === 'relativePrifix') {
      if (!value) {
        setInputData({ ...inputData, relativePrifix: '', relativePrifixId: null });
      } else {
        const selectedPrefix = prefix.find((val) => val._id === value);
        setInputData({
          ...inputData,
          relativePrifix: selectedPrefix ? selectedPrefix.prefix : '',
          relativePrifixId: value,
        });
      }
    }
  ...
  const handleBasicInfo = () => {
    ...
    // validateFields(mobile_no, aadhar_no);
    // generateUHID();
  ...
  const handlePatientSubmit = async (event) => {
    ...
    const { abha_card, aadhar_card, cardAttachment, ...otherData } = inputData;
    ...
    try {
      ...
      const response = await fetch(`${apiUrlMapping[formType]}/${patientDetail?._id}`, {
        method: 'PUT',
        headers: {
      ...
      toast.success('Patient Information Edited successfully!', {
        autoClose: 1000 // time in milliseconds (1 sec)
      });
  ...
  const opdBilling = useSelector((state) => state.opdBilling);
  ...
  return (
    <div className="wideModal" style={{ height: `${basicInfoFilled ? '100vh' : '80vh'}`, margin: '3rem' }}>
      <form onSubmit={handlePatientSubmit}>
        <h2 style={{ textAlign: 'center' }}>{formType} Registration</h2>
      ...
      <Modal open={opdBillingModal} onClose="">
        <OPDBilling close={setOpenBillingModal} billingData={billingData} />
      </Modal>
...
export default EditConfirmAppointment;
