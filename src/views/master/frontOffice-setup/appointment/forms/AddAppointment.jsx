...
const AddAppointment = ({ getData, close, margin, isOpdDashboard, selectedSlot, drInfo } = {}) => {
  const formattedDate = drInfo?.date
    ? new Date(drInfo.date).toISOString().split('T')[0] // Convert to YYYY-MM-DD
  ...
  const [inputData, setInputData] = useState({
    ...
    consultantId: '',
    time: selectedSlot || '',
    date: isOpdDashboard ? formattedDate : '',
    consultationType: '',
    appointmentMode: '',
    slot: ''
  });
  ...
  useEffect(() => {
    Promise.all([
      get('appointmentSchedule-master').then((res) => setDoctorAvailable(res.data)),
      get('opd').then((res) => setAlreadyBookAppointment(res.data)),
  ...
  const handleChange = (event) => {
    ...
    if (name === 'prefix') {
      ...
      const dep = consultantBook
        ? `${consultantBook.basicDetails.firstName} ${consultantBook.basicDetails.middleName} ${consultantBook.basicDetails.lastName}`
        : '';
  ...
  const handlePatientSubmit = async (event) => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        const response = await post('patient-appointment', inputData);
  ...
  const handleChipClick = (slot) => {
    ...
    setOpenBookTime(false); // Close the modal after selecting a slot
  };
  ...
  useEffect(() => {
    if (drInfo) {
      const dep = departments?.find((v) => v._id === drInfo?.departmentId) || '';
      const autoSelectByDepartment = consultanta?.filter((v) => v.employmentDetails?.departmentOrSpeciality?._id === drInfo?.departmentId);
      const cons = consultanta.find((v) => v._id === drInfo?.consultantId);
      const d = cons ? `${cons?.basicDetails?.firstName} ${cons?.basicDetails?.middleName} ${cons?.basicDetails?.lastName}` : '';
      setConsultant(autoSelectByDepartment ?? []);
      ...
      setInputData({
        ...
        date: new Date(drInfo?.date).toISOString().split('T')[0] || new Date().toISOString().split('T')[0],
        time: selectedSlot || ''
      });
    }
  }, [drInfo, departments, consultanta]);

  // useEffect(() => {
  //   filteredSlots();
  // }, [inputData.date, inputData.departmentId, inputData.consultantId]);
  ...
  return (
    <div>
      <div className="modal" style={{ height: '85vh', marginTop: margin }}>
        ...
        <form onSubmit={handlePatientSubmit}>
          ...
          <Grid container spacing={2} mt={1}>
          <Grid item xs={12} sm={6}>
  <FormControl fullWidth error={!!err.departmentName}>
    <Autocomplete
      options={departments}
      getOptionLabel={(option) => option.departmentName || ''}
      value={departments.find((dept) => dept._id === inputData.departmentId) || null}
      onChange={(event, newValue) => {
        handleChange({ target: { name: 'departmentName', value: newValue?._id || '' } });
      }}
      renderInput={(params) => (
        <TextField
          {...params}
    ...
    />
            ...
            <Grid item xs={12}>
              <div className="btnGroup">
                <Button color="primary" type="submit" title="Save">
                  <FaSave style={{ fontSize: '1.5rem' }} />
                </Button>
                <Button title="Cancel" style={{ backgroundColor: '#e74c3c' }} onClick={closeRegistration} className="btnCancel">
                  <Cancel />
                </Button>
              </div>
            </Grid>
