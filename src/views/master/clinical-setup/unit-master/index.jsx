...
const Unit = () => {
  ...
  const fetchData = async () => {
    setLoader(true);
    const response = await get('clinical-setup/units');
    setData(response?.units || []);
    setFilteredData(response?.units || []);
    setLoader(false);
  };
  ...
  const handleDelete = async () => {
    if (openDeleteModal) {
      const response = await put(`clinical-setup/units/delete/${deleteId}`);
      if (response) {
  ...
  useEffect(() => {
    const filtered = data?.filter((item) => item?.unit?.toLowerCase()?.includes(searchTerm?.toLowerCase()));
          ...
          <Card>
            ...
            <Modal open={openModal} onClose={closeModal}>
              <Box style={{ backgroundColor: 'white', margin: 'auto', marginTop: '10%', maxWidth: '400px' }}>
                {type === 'add' ? (
                  <AddUnit handleClose={closeModal} getData={fetchData} />
                ) : (
                  <EditUnit handleClose={closeModal} editData={editData} getData={fetchData} />
                )}
              </Box>
            </Modal>
