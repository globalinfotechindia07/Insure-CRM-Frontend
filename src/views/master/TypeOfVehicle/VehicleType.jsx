...
const VehicleType = () => {
  ...
  const validate = () => {
    ...
    if (!form.vehicleType || form.vehicleType.trim() === '') {
      newErrors.vehicleType = 'Vehicle Type Name is required';
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editIndex !== null) {
        ...
        await put(`vehicleType/${id}`, { vehicleType: form.vehicleType });
        toast.success('Record Edited Successfully');
      } else {
        // Create new
        await post('vehicleType', { vehicleType: form.vehicleType });
  ...
  const exportCSV = () => {
    ...
    const headers = ["Vehicle Type"];
    let csvContent = headers.join(",") + "\n";
    data.forEach(item => {
      csvContent += `"${(item.vehicleType || '').replace(/"/g, '""')}"\n`;
    });
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", "vehicle_types.csv");
  ...
  const handleImportCSV = (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      if (!header.includes("vehicle type")) {
        toast.error("Invalid CSV format. Header must contain 'Vehicle Type'");
        return;
      }
      ...
      const existingSet = new Set(data.map(item => (item.vehicleType || '').toLowerCase().trim()));
      const uniqueNew = [...new Set(imported)].filter(v => !existingSet.has(v.toLowerCase().trim()));

      if (uniqueNew.length === 0) {
        toast.info("No new unique vehicle types found to import.");
        return;
      }
      ...
      for (const val of uniqueNew) {
        try {
          const res = await post("vehicleType", { vehicleType: val });
          if (res) {
            successCount++;
          }
        } catch (err) {
          console.error(`Failed to import vehicle type: ${val}`, err);
        }
      }
      ...
      if (successCount === 0) {
        ...
        toast.success(`Imported ${successCount} new unique vehicle types successfully!`);
      }
      fetchVehicleTypes();
    };
    ...
    <div>
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle sx={{ m: 0, p: 2 }}>
          {editIndex !== null ? 'Edit Vehicle Type' : 'Add Vehicle Type'}
          <IconButton
            aria-label="close"
            onClick={handleClose}
            sx={{
