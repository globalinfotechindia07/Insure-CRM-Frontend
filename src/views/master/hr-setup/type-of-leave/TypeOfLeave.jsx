...
import AddTypeOfLeave from './forms/AddTypeOfLeave'
import EditTypeOfLeave from './forms/EditTypeOfLeave'
...
const TypeOfLeave = () => {
  ...
  useEffect(() => {
    const filtered = data.filter(item => item.typeOfLeave.toLowerCase().includes(searchTerm.toLowerCase()))
    setFilteredData(filtered)
  }, [searchTerm, data])

  const handleSave = async () => {
    if (openDeleteModal) {
      const response = await put(`typeOfLeave/delete/${deleteId}`)
  ...
  const columns = ['SN', 'Type of Leave', 'Action']
  ...
  return (
    <>
      <Breadcrumb title='Type of Leave'>
          ...
          <Card>
                ...
                <Grid container alignItems='center' justifyContent='space-between'>
                  ...
                  <Grid item>
                    <TextField
                      ...
                      label='Search'
                      variant='outlined'
                      placeholder='Search Type of Leave'
                      value={searchTerm}
            ...
            <Modal open={openModal} onClose={closeModal}>
              <Box style={{ backgroundColor: 'white', margin: 'auto', marginTop: '10%', maxWidth: '400px' }}>
                {type === 'add' ? (
                  <AddTypeOfLeave handleClose={closeModal} getData={fetchData} />
                ) : (
                  <EditTypeOfLeave handleClose={closeModal} editData={editData} getData={fetchData} />
                )}
              </Box>
            </Modal>
            <Modal open={openDeleteModal} onClose={() => setOpenDeleteModal(false)}>
              <Box p={3} style={{ backgroundColor: 'white', margin: 'auto', marginTop: '15%', maxWidth: '400px' }}>
                <Typography>Are you sure you want to delete this Type of Leave?</Typography>
