...
import AddGraduation from './forms/AddGraduation'
import EditGraduation from './forms/EditGraduation'
...
const Graduation = () => {
  ...
  useEffect(() => {
    const filtered = data.filter(item => item.graduation.toLowerCase().includes(searchTerm.toLowerCase()))
    setFilteredData(filtered)
  }, [searchTerm, data])

  const handleSave = async () => {
    if (openDeleteModal) {
      const response = await put(`graduation/delete/${deleteId}`)
  ...
  const columns = ['SN', 'Graduation', 'Action']
          ...
          <Card>
            ...
            <Modal open={openModal} onClose={closeModal}>
              <Box style={{ backgroundColor: 'white', margin: 'auto', marginTop: '10%', maxWidth: '400px' }}>
                {type === 'add' ? (
                  <AddGraduation handleClose={closeModal} getData={fetchData} />
                ) : (
                  <EditGraduation handleClose={closeModal} editData={editData} getData={fetchData} />
                )}
              </Box>
            </Modal>
            <Modal open={openDeleteModal} onClose={() => setOpenDeleteModal(false)}>
              <Box p={3} style={{ backgroundColor: 'white', margin: 'auto', marginTop: '15%', maxWidth: '400px' }}>
                <Typography>Are you sure you want to delete this Graduation?</Typography>
