...
const EditGraduation = ({ handleClose, getData, editData }) => {
  ...
  const handleSubmitData = async e => {
    ...
    if (inputData.graduation === '') {
      setError(prev => ({ ...prev, graduation: 'Graduation name is required' }))
      return
    }

    if (inputData.graduation !== '') {
      try {
        const response = await put(`graduation/${editData._id}`, { inputData })
        console.log(response)
        if (response.success === true) {
          setInputData({ graduation: '' })
  ...
  return (
    <div className='modal'>
      <h2 className='popupHead'>Update Graduation</h2>
