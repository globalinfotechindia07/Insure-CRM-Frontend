...
import AddDiploma from './forms/AddDiploma'
import EditDiploma from './forms/EditDiploma'
...
const Diploma = () => {
  ...
  useEffect(() => {
    const filtered = data.filter(item => item.diploma.toLowerCase().includes(searchTerm.toLowerCase()))
    setFilteredData(filtered)
  }, [searchTerm, data])

  const handleSave = async () => {
    if (openDeleteModal) {
      const response = await put(`diploma/delete/${deleteId}`)
  ...
  const columns = ['SN', 'Diploma', 'Action']
          ...
          <Card>
            ...
            <Modal open={openModal} onClose={closeModal}>
              <Box style={{ backgroundColor: 'white', margin: 'auto', marginTop: '10%', maxWidth: '400px' }}>
                {type === 'add' ? (
                  <AddDiploma handleClose={closeModal} getData={fetchData} />
                ) : (
                  <EditDiploma handleClose={closeModal} editData={editData} getData={fetchData} />
                )}
              </Box>
            </Modal>
            <Modal open={openDeleteModal} onClose={() => setOpenDeleteModal(false)}>
              <Box p={3} style={{ backgroundColor: 'white', margin: 'auto', marginTop: '15%', maxWidth: '400px' }}>
                <Typography>Are you sure you want to delete this Diploma?</Typography>
