...
const EditGipsaa = ({ close, getData, editData }) => {
  const [error, setError] = useState({});
  const [inputData, setInputData] = useState({
    gipsaaCompanyName: editData?.gipsaaCompanyName || ''
  });
  ...
  async function handleSubmit(e) {
    ...
    if (inputData.gipsaaCompanyName === '') {
      newError.gpisaaCompanyName = 'GIPSAA company name is required';
    }

    setError(newError);

    if (Object.keys(newError).length === 0) {
      await put(`gipsaa-company/update/${editData?._id}`, inputData)
        .then(() => {
          setInputData({
            gipsaaCompanyName: ''
          });
          toast.success('GIPSAA company Updated');
          close();
          getData();
        })
        .catch((error) => {
