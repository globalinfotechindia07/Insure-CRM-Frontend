...
const ParentCategory = () => {
  ...
  const headerFields = ['Charity Category'];
  ...
  const deleteParentGroup = async (id) => {
    await put(`category/parent-group/delete/${id}`)
      .then((response) => {
        fetchParentGroup();
  ...
  const handleSubmit = async (e) => {
    ...
    if (parentGroupName !== '') {
      await post('category/parent-group', { parentGroupName: parentGroupName })
        .then(() => {
          setOpenRegistrationModal(false);
          fetchParentGroup();
          setParentGroupName('');
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (parentGroupName !== '') {
      await put(`category/parent-group/${data._id}`, { parentGroupName: parentGroupName })
        .then(() => {
          setOpenEditModal(false);
          fetchParentGroup();
  ...
  const fileValidationHandler = (fileData) => {
    ...
    fileData?.forEach((val) => {
      ...
      if (val['Chaity Category'] !== '' && val['Charity Category'] !== undefined) {
        d = {
          parentGroupName: val['Charity Category']
        };
        newData.push(d);
      }
    });
  ...
  const exportDataHandler = () => {
    ...
    showData?.forEach((val, ind) => {
      data.push({
        SN: ind + 1,
        'Charity Category': val?.parentGroupName
      });
    });
    return data;
  };
  ...
  const columns = ['SN', 'Charity Category', 'Action'];
  const finalData =
    showData &&
    showData.map((item, ind) => {
      return {
        SN: ind + 1,
        'Charity Category': item.parentGroupName,
        Action: (
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: '', alignItems: 'center' }}>
            ...
            <ImportExport
              ...
              api="category/parent-group/import"
            />
          </div>
        </div>
