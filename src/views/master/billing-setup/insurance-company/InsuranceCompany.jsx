...
const InsuranceCompany = () => {
  ...
  const headerFields = ['Insurance Company Name'];
  const downheaderFields = ['Insurance Company Name'];
  ...
  const getData = async () => {
    setLoader(true);
    await get('insurance-company').then((response) => {
      let addsr = [];
      response.allInsuranceCompany.forEach((val, index) => {
  ...
  const filterData = (e) => {
    ...
    const filteredData = serverData.filter((item) => {
      item.insuranceCompanyName?.toLowerCase().includes(searchValue);
    });
    let addsr = [];
  ...
  const deleteData = async (id) => {
    await put(`insurance-company/delete/${id}`)
      .then(() => {
        getData();
        toast.error(`${data.insuranceCompanyName} Company Deleted`);
        setOpenDeleteModal(false);
      })
      .catch((error) => {
  ...
  const fileValidationHandler = (fileData) => {
    ...
    fileData?.forEach((val) => {
      if (val['Insurance Company Name'] !== undefined) {
        const d = {
          insuranceCompanyName: val['Insurance Company Name']
        };
        newData.push(d);
      }
    });
  ...
  const exportDataHandler = () => {
    ...
    showData.forEach((val, ind) => {
      datadd.push({
        'Insurance Company Name': val.insuranceCompanyName
      });
    });
    return datadd;
  };

  const columns = ['SN', 'Insurance Company Name', 'Action'];
  const finalData =
    showData &&
  ...
  return (
    <>
      <Breadcrumb title="Insurance Company">
      ...
      <Card>
        <CardContent>
          ...
          >
            <div className="modal">
              <h2 className="popupHead">Delete {data.insuranceCompanyName} company?</h2>
