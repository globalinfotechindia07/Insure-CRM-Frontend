...
const PatientPayee = () => {
  ...
  const fetchData = async () => {
    setLoader(true);
    await get('charity/patient-payee').then((response) => {
      let addsr = [];
    ...
    await get('charity/payee-group').then((response) => {
      setPayeeParent(response.data);
    });

    await get('charity/parent-group').then((response) => {
      setParentGroup(response.data);
    });
  };

  const deleteData = async (id) => {
    await put(`charity/patient-payee/delete/${id}`)
      .then(() => {
        fetchData();
  ...
  const validation = () => {
    const fieldsToValidate = [
      ...
      { field: 'parentGroup', message: 'Payee Category is required' },
  ...
  const handleSubmit = async (e) => {
    ...
    if (isFormValid) {
      await post('charity/patient-payee', inputData)
        .then(() => {
          setOpenRegistrationModal(false);
          fetchData();
          setInputData({
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (isFormValid) {
      await put(`charity/patient-payee/${data._id}`, inputData)
        .then(() => {
          setOpenEditModal(false);
          fetchData();
    ...
    fileData.forEach((val) => {
      ...
      if (
        ...
        d = {
          ...
          parentGroup: val['Payee Category'],
          address: val['Address'],
          licNo: val['Lic NO'],
  ...
  const exportDataHandler = () => {
    ...
    showData?.forEach((val, ind) => {
      data?.push({
        ...
        'Payee Category': val?.parentGroup,
  ...
  const getData = async () => {
    ...
    try {
      await axios
        .get(`${REACT_APP_BASE_URL}charity/patient-payee`, {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: '', alignItems: 'center' }}>
            ...
            <ImportExport
              ...
              api="charity/patient-payee/import"
            />
          </div>
        </div>
