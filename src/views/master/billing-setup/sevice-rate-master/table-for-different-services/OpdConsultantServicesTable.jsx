...
function OpdConsultantServicesTable () {
  const [opdConsultantServicesData, setOpdConsultantServicesData] = useState([])
  ...
  // Fetch OPD consultant services data
  const fetchOpdConsultantServices = async () => {
    setIsLoading(true)
    try {
      const services = await get('opd-consultant-service/all')
      setOpdConsultantServicesData(services?.data?.length > 0 ? services.data : [])
      setFilteredData(services?.data?.length > 0 ? services.data : []) // Initialize filtered data
    } catch (error) {
      console.error('Error fetching OPD consultant services:', error)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSearch = debounce(term => {
    const filtered = opdConsultantServicesData.filter(item => {
      return (
        item.consultantName.toLowerCase().includes(term.toLowerCase()) ||
        item.type.toLowerCase().includes(term.toLowerCase()) ||
        item.billGroup.toLowerCase().includes(term.toLowerCase()) ||
...
export default OpdConsultantServicesTable
