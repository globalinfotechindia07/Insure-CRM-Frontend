...
function RadiologyServiceTable () {
  const [radiologyServicesData, setRadiologyServicesData] = useState([])
  ...
  // Fetch radiology services data
  const fetchRadiologyServices = async () => {
    setIsLoading(true)
    try {
      const services = await get('investigation-radiology-master')
      setRadiologyServicesData(services.investigation || [])
      setFilteredData(services.investigation || []) // Initialize filtered data
    } catch (error) {
  ...
  const handleSearch = debounce(term => {
    const filtered = radiologyServicesData.filter(item => {
      return (
...
export default RadiologyServiceTable
