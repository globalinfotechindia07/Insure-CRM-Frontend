...
function OtherDiagnosticsServicesTable() {
  const [otherDiagnosticsServicesData, setOtherDiagnosticsServicesData] = useState([]);
  ...
  // Fetch other diagnostics services data
  const fetchOtherDiagnosticsServices = async () => {
    setIsLoading(true);
    try {
      const services = await get('other-diagnostics-master');
      setOtherDiagnosticsServicesData(services.diagnostics.length > 0 ? services.diagnostics : []);
      setFilteredData(services.diagnostics.length > 0 ? services.diagnostics : []); // Initialize filtered data
    } catch (error) {
      console.error('Error fetching other diagnostics services:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSearch = debounce((term) => {
    const filtered = otherDiagnosticsServicesData.filter((item) => {
      return (
...
export default OtherDiagnosticsServicesTable;