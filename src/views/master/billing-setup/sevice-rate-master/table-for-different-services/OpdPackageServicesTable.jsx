...
function OpdPackageServicesTable () {
  const [opdPackageServicesData, setOpdPackageServicesData] = useState([])
  ...
  const fetchOpdPackageServices = async () => {
    setIsLoading(true)
    try {
      const services = await get('opd-package')
      if (services.package.length > 0) {
        setOpdPackageServicesData(services.package || [])
      } else {
        setOpdPackageServicesData([])
      }
    } catch (error) {
      console.error('Error fetching OPD package services:', error)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const filteredData = opdPackageServicesData.filter(item => {
    return (
      item.services.some(service => service.detailServiceName.toLowerCase().includes(searchTerm.toLowerCase())) ||
      item.billGroup?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.serviceGroupOrBillGroup?.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <TableBody>
            {currentRows.map((item, index) => (
              <TableRow key={item._id} sx={{ backgroundColor: isValidStates[item._id] !== false ? '' : '#fab57a' }}>
                ...
                <TableCell align='center' style={styles.td}>
                  {item.services.length > 0 ? item.services.map(service => service.detailServiceName).join(', ') : 'No services found'}
                </TableCell>
                <TableCell align='center' style={styles.td}>
                  {item.department}
                </TableCell>
...
export default OpdPackageServicesTable
