...
const AddServiceRate = ({close,fetchData}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      try {
        const response = await post("service-rate", inputData);
        toast.success(response.msg || "Service rate created successfully");
        setInputData({
          serviceGroup: "",
          serviceGroupId: "",
          serviceName: "",
  ...
  return (
    <div className="modal">
      <form onSubmit={handleSubmit} style={{ padding: "20px" }}>
        <h2 style={{ marginBottom: "1rem" }}>Add Service Rate</h2>
        {loading ? (
          <CircularProgress />
        ) : (
