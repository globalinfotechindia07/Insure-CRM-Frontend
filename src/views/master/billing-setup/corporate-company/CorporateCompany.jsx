...
const CorporateCompanyPublic = () => {
  ...
  const headerFields = ['Corporate Company Public Name'];
  const downheaderFields = ['Corporate Company Public Name'];
  ...
  const getData = async () => {
    setLoader(true);
    await get('insurance-company/co-operative').then((response) => {
      let addsr = [];
  ...
  const deleteData = async (id) => {
    await put(`insurance-company/co-operative/delete/${id}`)
      .then(() => {
        getData();
  ...
  const fileValidationHandler = (fileData) => {
    ...
    fileData?.forEach((val) => {
      let d = {};
      if (val['Corporate Company Public Name'] !== undefined) {
        d = {
          cooperativeCompanyName: val['Corporate Company Public Name']
        };
        newData.push(d);
      }
    });
  ...
  const exportDataHandler = () => {
    ...
    showData.forEach((val, ind) => {
      datadd.push({
        SN: ind + 1,
        'Corporate Company Public Name': val.cooperativeCompanyName
      });
    });
    return datadd;
  };

  const columns = ['SN', 'Corporate Company Public', 'Action'];
  const finalData =
    showData &&
    showData.map((item, ind) => {
      return {
        SN: ind + 1,
        'Corporate Company Public': item.cooperativeCompanyName,
        Action: (
  ...
  return (
    <>
      <Breadcrumb title="Corporate Company Public">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', justifyContent: '', alignItems: 'center' }}>
              ...
              <ImportExport
                ...
                name="corporate-company-public"
                fileValidationHandler={fileValidationHandler}
                exportDataHandler={exportDataHandler}
                api="insurance-company/co-operative/import"
              />
            </div>
          </div>
