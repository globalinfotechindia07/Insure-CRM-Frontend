...
const TicketDetailView = () => {
  ...
  const fetchTicketDetails = async () => {
    try {
      const response = await get(`ticket-management/${id}`);
      if (response) {
        setTaskData(response);
        setStatusChanged(response.status);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await put(`ticket-management/${id}`, statusData);
                ...
                {showDetails && (
                  <Box sx={{ border: '1px solid #e0e0e0', borderTop: 'none', p: 1 }}>
                    {[
                      {
                        label: 'Assignee',
                        value: taskData?.AssignTo.basicDetails.firstName + ' ' + taskData?.AssignTo.basicDetails.lastName
                      },
                      { label: 'Priority', value: taskData?.urgency, isChip: true, chipColor: '#f43f5e' },
                      { label: 'Product', value: taskData?.product, isChip: true },
                      { label: 'Phone Number', value: taskData?.phoneNumber },
                      { label: 'Service Type', value: taskData?.serviceType }
                    ].map((field, index) => (
                    ...
                    <Grid container py={1}>
                      ...
                      <Grid item xs={7}>
                        <Chip
                          label={new Date(taskData?.installDate).toLocaleDateString('en-US')}
                          size="small"
                    ...
                    <Grid container py={1}>
                      ...
                      <Grid item xs={7}>
                        <Chip
                          label={new Date(taskData?.expiryDate).toLocaleDateString('en-US')}
                          size="small"
                          sx={{ bgcolor: '#9ca3af', color: 'white', fontWeight: 500 }}
                        />
                      </Grid>
          ...
          <Grid item xs={12} md={8}>
            <Card sx={{ border: '1px solid #e0e0e0' }}>
              <CardContent>
                ...
                <Box sx={{ mb: 2, display: 'inline-block', bgcolor: '#10b981', borderRadius: '6px', px: 1.5, py: 0.5 }}>
                  ...
                  >
                    {statusOptions.map((option) => (
                      <MenuItem key={option._id || option.TicketStatus} value={option.TicketStatus}>
                        {option.TicketStatus}
                      </MenuItem>
                    ))}
                  </TextField>
                ...
                >
                  <Stack spacing={2}>
                    {[...statusHistory].reverse().map((item, idx) => (
                        ...
                        <Box display="flex" gap={2}>
                          ...
                          <Box flex={1}>
                            ...
                            <Box display="flex" alignItems="center" gap={1}>
                              ...
                              <Typography variant="body2" color="#6366f1" fontWeight={500}>
                                {taskData?.AssignTo.basicDetails.firstName + ' ' + taskData?.AssignTo.basicDetails.lastName}
                              </Typography>
