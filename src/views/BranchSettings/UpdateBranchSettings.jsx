...
const UpdateBranchSettings = () => {
  ...
  useEffect(() => {
    const fetchBranch = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://api.jpinsurancebrokers.co.in/api/branchSettings/${id}`, {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let url = 'http://api.jpinsurancebrokers.co.in/api/branchSettings/';

      let method = 'POST';
      
      if (!isNewRecord) {
        // url = `http://localhost:5050/api/branchSettings/${id}`;
        url = `http://api.jpinsurancebrokers.co.in/api/branchSettings/${id}`;
      ...
      if (data.status === 'true') {
        ...
        if (logoFile) {
          ...
          const logoResponse = await fetch(`http://api.jpinsurancebrokers.co.in/api/branchSettings/${data.data._id}/logo`, {
