  ...
  const handleSubmit = async (e) => {
    ...
    if (storedAllData?.submittedFormId) {
      try {
        ...
        const updateDocuments = await put(`administrative/documents/${storedAllData?.submittedFormId}`, formData);

        console.log('file document uploaded: ', updateDocuments);
        ...
        // }
        if (updateDocuments.success === true) {
          setStoredAllData((prev) => ({ ...prev, documentation: files }));
          toast.success(updateDocuments.message);
          setValue((prev) => prev + 1);
        }

        if (updateDocuments.success === false) {
          toast.error(updateDocuments.message);
        }
      } catch (error) {
