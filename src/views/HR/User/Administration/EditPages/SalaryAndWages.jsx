...
function SalaryAndWages({ setValue, storedAllData, setStoredAllData }) {
  ...
  //   // console.log('currentMonth:', currentMonth, 'ptData:', ptData);
  ...
  const calculateEmployeeDeductionTotal = (salary) => {
    ...
    // Calculate employee deductions from entry data
    const entryDeductions = deductData.data.reduce((total, item) => {
      const baseAmount = calculateBaseAmount(item, salary);

      console.log('Base Salary:', baseSalary);
      console.log("Deduction Component for PT:", item); // Log the current deduction component

      console.log('Deduction percentage (employee):', item.employee);

      const employeeDeduction = (item.employee * baseAmount) / 100;
      console.log('Employee Deduction:', employeeDeduction);
      console.log('Calculated Employee Deduction: ', employeeDeduction);

      return total + employeeDeduction;
    }, 0);

    // Add PT deduction (if any)
    const currentMonth = new Date().toLocaleString('default', { month: 'long' }).toLowerCase();
    const currentPT = ptData.find((item) => item.month?.toLowerCase() === currentMonth);
    const ptAmount = currentPT ? Number(currentPT.amount) : 0;

    // Total employee deductions
    const totalDeductions = entryDeductions + ptAmount;

    // Cap the deductions if they exceed the gross income
    const cappedDeductions = Math.min(totalDeductions, salaryDetails.totalIncomeMonthly);

    return cappedDeductions;
  };
  ...
  const calculateEmployerDeductionTotal = (salary) => {
    ...
    return deductData.data.reduce((total, item) => {
      ...
      return total + employerDeduction;
    }, 0);
  };
  ...
  const preparePayload = (salary) => {
    ...
    console.log('deductionComponents Professional:', JSON.stringify(deductionComponents, null, 2));
    console.log('deduction amount: ', deductionComponents.amount);
    console.log('deduction amount: ', deductionComponents.month);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('storeAllData empCode :', storedAllData.basicDetails.empCode);
      if (!id) {
            ...
            <TableContainer component={Paper}>
              <Table sx={{ tableLayout: 'fixed', width: '100%' }}>
                ...
                <TableBody>
                  ...
                  {/* todo: new data */}
                  {/* // Ensure this is properly set up for both Monthly and Annual Salary. */}
                  <TableRow>
  <TableCell sx={{ width: 20 }}>Basic Salary</TableCell>
  <TableCell align="right">
    {isEditing ? (
  ...
  <TableCell align="right">{formatCurrency((submittedSalary || baseSalary) * 12)}</TableCell>
</TableRow>


                 {incomeData.map((item) => {
  const salary = isEditing ? baseSalary : submittedSalary;
  const monthly = item.amount ? (item.amount * salary) / 100 : 0; // Ensuring no calculation error if amount is null
  return (
  ...
  const employeePercentage = item.employee || 0; // Default to 0 if undefined/null
  const employerPercentage = item.employer || 0; // Default to 0 if undefined/null

  const employeeDeduction = (employeePercentage * baseAmount) / 100;
  const employerDeduction = (employerPercentage * baseAmount) / 100;
      ...
      <TableCell align="right">
        {`${formatCurrency(employerDeduction)} (${formatCurrency(employerDeductionAnnual)})`}
                  ...
                  <TableRow>
                    <TableCell>PT</TableCell>
                    <TableCell align="right">
                      {formatCurrency(ptAmount)} ({formatCurrency(annualPTAmount)})
                    </TableCell>
                    <TableCell align="right">-</TableCell>
                  </TableRow>
  ...
  <TableCell align="right">
    <b>
      {`${formatCurrency(salaryDetails.employeeDeductionsMonthly || 0)} (${formatCurrency(
        (salaryDetails.employeeDeductionsMonthly - ptAmount) * 12 + annualPTAmount
      )})`}
    </b>
  </TableCell>
  <TableCell align="right">
    <b>
      {`${formatCurrency(salaryDetails.employerDeductionsMonthly || 0)} (${formatCurrency(
        salaryDetails.employerDeductionsMonthly * 12
      )})`}
    </b>
  </TableCell>
</TableRow>
          ...
          <Grid item xs={12} md={6}>
            <Box display="flex" flexDirection="column" alignItems="flex-start" gap={1} p={3} borderRadius={3} boxShadow={2} bgcolor="white">
              ...
              <Typography variant="h6">
                ...
                {formatCurrency(salaryDetails.employeeDeductionsMonthly)}) ={' '}
                {formatCurrency(salaryDetails.totalIncomeMonthly - salaryDetails.employeeDeductionsMonthly)}
              </Typography>
            </Box>
          </Grid>
