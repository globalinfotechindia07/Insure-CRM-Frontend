...
const SupportMainPage = () => {
  ...
  const fetchSupportData = async () => {
    setLoading(true)
    try {
      const response = await get('support')
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await put(`support/delete/${selectedItem._id}`)

      if (response?.success) {
        // Use `message` from the backend response for success toast
        toast.success(response?.message || 'Item deleted successfully')
        fetchSupportData() // Refresh data after successful deletion
      } else {
                        ...
                        <TableBody>
                          {filteredData.map((item, index) => (
                            <TableRow key={item._id}>
                              ...
                              <TableCell>
                                <EditBtn onClick={() => navigate(`/users/edit-support/${item._id}`)} />
                                <DeleteBtn onClick={() => handleDeleteClick(item)} />
                                <ViewBtn onClick={() => navigate(`/users/viewUserDetails/support/${item._id}`)} />
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
