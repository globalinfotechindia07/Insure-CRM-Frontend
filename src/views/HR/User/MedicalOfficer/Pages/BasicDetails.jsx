...
const BasicDetails = ({ setValue, storedAllData, setStoredAllData }) => {
  ...
  const [latestEmployeeCode, setLatestEmployeeCode] = useState('')
      async function fetchLatestEmployeeCode () {
        const response = await get('medicalOfficer/medicalOfficerData/getEmployeeCode')
  ...
  const handleSubmit = async event => {
    ...
    if (isValid) {
      ...
      const submitBasicDetails = await fetch(`${REACT_APP_API_URL}medicalOfficer/basicDetails`, {
        method: 'POST',
        body: formData
      })
