...
function EmploymentDetails({ setValue, setStoredAllData, storedAllData }) {
  ...
  const [medicalOfficerDataForReportTo, setMedicalOfficerDataForReportTo] = useState([]);

  async function fetchMedicalOfficerDataForReportTo() {
    const response = await get('medicalOfficer/medicalOfficerData/reportTo');
    setMedicalOfficerDataForReportTo(response.data.length > 0 ? response.data : []);
  }
  ...
  useEffect(() => {
    ...
    fetchEmployeeRole();
    fetchDesignation();
    fetchMedicalOfficerDataForReportTo();
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log('MEDICAL OFFICERS', employmentDetails);
    // if (employmentDetailValidation()) {
    if (true) {
      if (storedAllData.submittedFormId) {
        const response = await put(`medicalOfficer/employmentDetails/${storedAllData?.submittedFormId}`, { employmentDetails });
          ...
          <form onSubmit={handleSubmit}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={4}>
                ...
                >
                  {medicalOfficerDataForReportTo && medicalOfficerDataForReportTo.length > 0 ? (
                    medicalOfficerDataForReportTo.map((item) => (
                      <MenuItem key={item._id} value={item._id}>
