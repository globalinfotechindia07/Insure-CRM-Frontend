...
const MedicalOfficerMainPage = () => {
  ...
  const goToAddPage = () => {
    navigate('/users/add-medical-officer');
  };

  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await get('medicalOfficer');
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await put(`medicalOfficer/delete/${selectedItem._id}`);
                        ...
                        <TableBody>
                          {filteredData.map((item, index) => (
                            <TableRow key={item._id}>
                              ...
                              <TableCell>
                                <EditBtn onClick={() => navigate(`/users/edit-medical-officer/${item._id}`)} />
                                <DeleteBtn onClick={() => handleDeleteClick(item)} />
                                <ViewBtn onClick={() => navigate(`/users/viewUserDetails/medicalOfficer/${item._id}`)} />
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
...
export default MedicalOfficerMainPage;
