...
import Qualification from './EditPages/Qualification';
...
const EditMedicalOfficer = () => {
  const [value, setValue] = useState(0);
  const { id } = useParams();
  const navigate = useNavigate();

  const fetchSingleMedicalOfficerData = async () => {
    const response = await get(`medicalOfficer/${id}`); // Update endpoint for Medical Officer
    console.log(response);
    setStoredAllData(response.data);
  };

  useEffect(() => {
    fetchSingleMedicalOfficerData();
  }, [id]);
  ...
  const renderMasterContent = () => {
    switch (value) {
      ...
      case 3:
        return <Qualification setStoredAllData={setStoredAllData} setValue={setValue} storedAllData={storedAllData} />;
      case 4:
  ...
  return (
    <>
      ...
      <Button sx={{ mb: 2 }} variant="contained" onClick={() => navigate('/users/medical-officer')}>
        Back
      </Button>
      <Box
        sx={{
...
export default EditMedicalOfficer;
