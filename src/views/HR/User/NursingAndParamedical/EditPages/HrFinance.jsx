...
function HRFinance ({ setValue, storedAllData, setStoredAllData }) {
  ...
  useEffect(() => {
    const hrData = storedAllData.hrFinance || {}
    setHrFinance(hrData)
    if (hrData.cancelCheck) {
      setIsExistingFile(true)
      setViewFileUrl(`${REACT_APP_API_URL}images/${hrData.cancelCheck}`)
    }
  ...
  const handleFileSelect = e => {
    ...
    if (file instanceof File) {
      ...
      setViewFileUrl(URL.createObjectURL(file)) // Create a URL for the newly uploaded file
    }
  }
  ...
  const handleSubmit = async e => {
    ...
    const submitHrFDetails = await fetch(`${REACT_APP_API_URL}nursingAndParamedical/hrFinance/${id}`, {
      method: 'PUT',
      body: formData
    })
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={4}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    ...
                    {hrFinance.cancelCheck instanceof File && (
                      <Typography variant='body2' color='textSecondary'>
                        Selected File: {hrFinance.cancelCheck.name}
                      </Typography>
                    )}
                    {viewFileUrl && !(hrFinance.cancelCheck instanceof File) && (
                      <Button
                        variant='contained'
                        color='primary'
                        onClick={() => window.open(viewFileUrl, '_blank')}
                        sx={{ width: 'auto', maxWidth: '200px', padding: '0 10px' }} // Flexible width with max limit
                      >
                        View Document
                      </Button>
                    )}
