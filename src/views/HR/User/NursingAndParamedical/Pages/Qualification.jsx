...
function Qualification({ setValue, setStoredAllData, storedAllData }) {
  ...
  useEffect(() => {
    setQualificationForm(
      Object.keys(storedAllData.qualification).length > 0
  ...
  useEffect(() => {
    setAdditionalDetailsForm(
      Object.keys(storedAllData.additionalDetails).length > 0
  ...
  const submitQualification = async (e) => {
    ...
    if (true) {
      if (storedAllData.submittedFormId) {
        const response = await put(`nursingAndParamedical/qualification/${storedAllData?.submittedFormId}`, { qualificationForm });
  ...
  const submitAdditionalDetails = async (e) => {
    ...
    if (true) {
      if (storedAllData.submittedFormId) {
        const response = await put(`nursingAndParamedical/additionalDetails/${storedAllData?.submittedFormId}`, { additionalDetailsForm });
        if (response.success === true) {
          setStoredAllData((prev) => ({
            ...prev,
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <Button type="submit" variant="contained" color="primary" disabled={storedAllData?.qualification?._id}>
                    Save
                  </Button>
                </Box>
              </Grid>
        ...
        <CardContent>
          ...
          <Box sx={{ marginTop: 4, display: 'flex', justifyContent: 'flex-end' }}>
            <Button
              ...
              disabled={storedAllData?.additionalDetails?._id}
            >
              Save
            </Button>
          </Box>
