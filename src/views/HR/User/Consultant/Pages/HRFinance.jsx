  ...
  const handleSubmit = async e => {
    ...
    if (true) {
      if (storedAllData?.submittedFormId) {
        ...
        const submitHrFDetails = await fetch(`${REACT_APP_API_URL}newConsultant/hrFinance/${storedAllData.submittedFormId}`, {
          method: 'PUT',
          body: formData
        })
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <Button type='submit' variant='contained' color='primary' disabled={storedAllData?.hrFinance?._id}>
                      Save & Next
                    </Button>
                  </Box>
                </Grid>
