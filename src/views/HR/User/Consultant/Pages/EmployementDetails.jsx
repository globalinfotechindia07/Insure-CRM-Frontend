...
function EmploymentDetails ({ setValue, setStoredAllData, storedAllData }) {
  ...
  async function fetchConsultantReportToData () {
    const response = await get('newConsultant/consultantData/reportTo')
    setConsultantReportTo(response.data.length > 0 ? response.data : [])
  }
  ...
  const handleSubmit = async e => {
    ...
    if (true) {
      if (storedAllData.submittedFormId) {
        const response = await put(`newConsultant/employmentDetails/${storedAllData?.submittedFormId}`, { employmentDetails })
          ...
          <form onSubmit={handleSubmit}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={4}>
                ...
                >
                  {consultantReportTo && consultantReportTo.length > 0 ? (
                    consultantReportTo.map(item => (
                      <MenuItem key={item._id} value={item._id}>
