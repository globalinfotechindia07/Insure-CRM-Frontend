...
const FindingsComponent = ({ departmentId, getOpthalmicData, allReadyOpthalmicId }) => {
  ...
  const handleSave = async () => {
    try {
      const findingData = { findings: selectedValues };
      let res;
      if (allReadyOpthalmicId?.finding) {
        res = await put(`patient-opthelmic/findings/${allReadyOpthalmicId?.finding}`, findingData);
      } else {
        res = await post(`patient-opthelmic/findings`, {
      ...
      if (res?.success) {
        toast.success(allReadyOpthalmicId?.finding ? 'Updated Successfully' : 'Created Successfully');
        getOpthalmicData();
        setSelectedValues({
          iopMethod: '',
          iopRE: '',
  ...
  const handleDeleteOption = async (optionToDelete) => {
    await deleteFindingOptions({
      finding: optionToDelete,
      category: currentCategory,
      eye: currentEye
    });
  };
                    ...
                    color={
                      currentCategory === 'color'
                        ? selectedValues[`${currentCategory}${currentEye}`]?.includes(option)
                          ? 'primary'
                          : 'default'
                        : selectedValues[selectedField] === option
                          ? 'primary'
                          : 'default'
                    }
