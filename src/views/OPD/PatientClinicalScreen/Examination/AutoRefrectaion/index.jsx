...
const AutoRefractionComponent = ({ departmentId, getOpthalmicData, allReadyOpthalmicId }) => {
  ...
  const handleSave = async () => {
    try {
      const payload = { autoRefraction: selectedValues };

      if (allReadyOpthalmicId?.autoRef) {
        const res = await put(`patient-opthelmic/auto-refraction/${allReadyOpthalmicId.autoRef}`, payload);
        if (res?.success) {
          toast.success('Updated Successfully');
          getOpthalmicData();
        }
      } else {
        const res = await post(`patient-opthelmic/auto-refraction`, {
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
        ...
        <DialogContent>
          <Grid container spacing={1}>
            ...
            {/* Your Chip rendering stays SAME */}
...
export default AutoRefractionComponent;
