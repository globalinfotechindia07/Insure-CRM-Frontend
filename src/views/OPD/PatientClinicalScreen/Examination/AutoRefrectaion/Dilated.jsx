...
const Dialated = ({ departmentId, getOpthalmicData, allReadyOpthalmicId }) => {
...
const [valueType, setValueType] = useState({
  ...
  const [addArUndilatedOption] = useAddArDilatedOptionMutation();
  ...
  console.log('OPTIO', fetchedOptions);
  async function handleSave() {
    try {
      if (allReadyOpthalmicId?.autoRef) {
        const res = await put(`patient-opthelmic/auto-refraction/${allReadyOpthalmicId?.autoRef}`, {
          autoRefraction: selectedValues
        });
    ...
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <Box sx={{ boxShadow: 3, padding: 3, backgroundColor: '#fff', borderRadius: 2 }}>
          ...
          >
            <span style={{ marginLeft: '10px' }}>AR (Auto Refraction) Dilated</span>
          </Typography>
          ...
          {/* Spherical, Cylinder, Axis Selection */}
