...
import BasicDetails from '../NursingAndTechnician/EditPages/BasicDetails'
import AddressDetails from '../NursingAndTechnician/EditPages/AddressDetails'
import PastEmploymentDetails from '../NursingAndTechnician/EditPages/PastEmploymentDetails'
import BankDetails from '../NursingAndTechnician/EditPages/BankDetails'
import EmploymentDetails from '../NursingAndTechnician/EditPages/EmploymentDetails'
import Documentation from '../NursingAndTechnician/EditPages/Documentation'
import REACT_APP_API_URL, { get, retrieveToken } from 'api/api'
...
const UpdatePage = () => {
  ...
  const fetchSingleAdministrativeData = async () => {
    const response = await get(`newConsultant/${id}`)
  ...
  const renderMasterContent = () => {
    switch (value) {
      case 0:
        return <BasicDetails setFormData={setFormData} setValue={setValue} formData={formData} />
      case 1:
        return <AddressDetails setFormData={setFormData} setValue={setValue} formData={formData} />
      case 2:
        return <PastEmploymentDetails setFormData={setFormData} setValue={setValue} formData={formData} />
      case 3:
        return <BankDetails setFormData={setFormData} setValue={setValue} formData={formData} />
      case 4:
        return <EmploymentDetails setFormData={setFormData} setValue={setValue} formData={formData} />
      case 5:
        return (
          <Documentation
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const response = await fetch(`${REACT_APP_API_URL}newConsultant/${id}`, {
        method: 'PUT',
        headers: {
    ...
    <>
      ...
      <Modal open={isModalOpen} onClose={handleModalClose} aria-labelledby='form-submission-modal'>
        ...
        >
          ...
          <Typography sx={{ mt: 2 }}>Are you sure you want to update the admininstrative data?</Typography>
