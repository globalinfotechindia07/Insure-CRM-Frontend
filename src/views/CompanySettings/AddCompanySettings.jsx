...
const CompanySettings = () => {
    ...
    if (window.confirm(`Are you sure you want to delete "${companyName}"?`)) {
      try {
        ...
        // const response = await fetch(`http://localhost:5050/api/companySettings/${companyId}`, {
          const response = await fetch(`http://api.jpinsurancebrokers.co.in/api/companySettings/${companyId}`, {
  ...
  const getLogoUrl = (logoPath) => {
    ...
    if (logoPath.startsWith('/uploads')) {
      ...
      const fullUrl = `http://api.jpinsurancebrokers.co.in${logoPath}`;
    ...
    return `http://api.jpinsurancebrokers.co.in/uploads/company-logo/${logoPath}`;
  ...
  const fetchCompanySettings = async () => {
    ...
    try {
      ...
      const response = await fetch('http://api.jpinsurancebrokers.co.in/api/companySettings/', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
